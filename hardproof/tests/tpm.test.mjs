...
import { LinuxTpm2ToolsProvider, TpmProver } from '../packages/prover-tpm/dist/index.js';
...
function fixture(options = {}) {
  ...
  return {
    ...
    verifier,
    deviceId,
    issue: () => verifier.nonceStore.issue(),
...
test('valid TPM quote fixture verifies as hardware rooted', () => {
  ...
  const result = value.verifier.verify(value.bundle(nonce), { nonce });
...
test('tampered PCR values are rejected against the signed composite', () => {
  ...
  const result = value.verifier.verify(bundle, { nonce });
...
test('accepted TPM proof cannot be replayed', () => {
  ...
  const nonce = value.issue();
  const bundle = value.bundle(nonce);
  assert.equal(value.verifier.verify(bundle, { nonce }).valid, true);
  const replay = value.verifier.verify(bundle, { nonce });
...
test('signed wrong PCR selection is rejected by verifier policy', () => {
  const value = fixture();
  const nonce = value.issue();
  const result = value.verifier.verify(value.bundle(nonce, { pcrSelection: [0, 8] }), { nonce });
  assert.equal(result.valid, false);
...
test('tampered or missing required event log is rejected', () => {
  ...
  const tamperedResult = tamperedValue.verifier.verify(tampered, { nonce: tamperedNonce });
...
test('unknown and expired verifier nonces are rejected', () => {
  ...
  const unknown = unknownValue.verifier.verify(unknownValue.bundle(unknownNonce), { nonce: unknownNonce });
  ...
  const expired = expiredValue.verifier.verify(expiredBundle, { nonce: expiredNonce });
...
test('real Linux TPM quote integration', { skip: process.env.HARDPROOF_REAL_TPM !== '1' }, async () => {
  assert.equal(process.platform, 'linux');
  const workDirectory = process.env.HARDPROOF_TPM_WORKDIR ?? await mkdtemp(join(tmpdir(), 'hardproof-tpm-'));
  const provider = new LinuxTpm2ToolsProvider({
    workDirectory,
    tcti: process.env.HARDPROOF_TPM_TCTI,
    akContextPath: process.env.HARDPROOF_TPM_AK_CONTEXT,
    akPublicKeyPath: process.env.HARDPROOF_TPM_AK_PUBLIC,
    akQualifiedNamePath: process.env.HARDPROOF_TPM_AK_QUALIFIED_NAME,
    normalizedEventLogPath: process.env.HARDPROOF_TPM_EVENT_LOG_JSON,
  });
  const ak = process.env.HARDPROOF_TPM_PROVISION_AK === '1'
  ...
  const verifier = new Verifier({
    ...
    requireTpmEventLog: Boolean(process.env.HARDPROOF_TPM_EVENT_LOG_JSON),
  });
