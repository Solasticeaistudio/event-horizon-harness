...
export class LinuxTpm2ToolsProvider {
    ...
    persistentHandle(value, fallback) {
        const handle = (value ?? fallback).toLowerCase();
        if (!/^0x81[0-9a-f]{6}$/.test(handle))
            throw new TypeError('TPM persistent handle must be in the 0x81xxxxxx range');
        return handle;
    }
    paths() {
        return {
    ...
    async provisionAk() {
        ...
        if (ekHandle === akHandle)
            throw new TypeError('EK and AK persistent handles must differ');
        ...
        for (const handle of providerHandles.reverse())
            this.run('tpm2_flushcontext', ['-Q', handle]);
        return this.loadAk();
    }
    async loadAk() {
        const paths = this.paths();
