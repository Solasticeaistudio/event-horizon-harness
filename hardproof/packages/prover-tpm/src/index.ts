...
export interface TpmQuoteProvider {
  prove(options: { deviceId: string; nonce: string; pcrSelection: number[] }): Promise<HardproofBundle>;
}
...
export class TpmProver {
  ...
  static async isAvailable(): Promise<boolean> {
    ...
    if (process.env.HARDPROOF_TPM_AVAILABLE === '1') return true;
  ...
  async prove(options: { nonce: string }): Promise<HardproofBundle> {
