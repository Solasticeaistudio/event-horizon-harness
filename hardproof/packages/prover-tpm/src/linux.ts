...
import type { HardproofBundle, HardproofBundleUnsigned } from '@hardproof/core';
...
export class LinuxTpm2ToolsProvider implements TpmQuoteProvider {
  ...
  async prove(options: { deviceId: string; nonce: string; pcrSelection: number[] }): Promise<HardproofBundle> {
    ...
    try {
      ...
      const unsigned: HardproofBundleUnsigned = {
        version: 'hp1',
        method: 'tpm2',
