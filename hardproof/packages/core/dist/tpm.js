import { constants, createHash, createPublicKey, createVerify, } from 'node:crypto';
...
class Cursor {
    ...
    take(size) {
        if (size < 0 || this.offset + size > this.data.length)
            throw new TypeError('truncated TPMS_ATTEST');
    ...
    sized(maximum = 4096) {
        const size = this.u16();
        if (size > maximum)
            throw new TypeError('TPM2B field exceeds limit');
...
export function parseTpmsAttest(input) {
        ...
        for (let byteIndex = 0; byteIndex < bitmap.length; byteIndex += 1) {
            for (let bit = 0; bit < 8; bit += 1) {
                if ((bitmap[byteIndex] & (1 << bit)) !== 0)
                    selectedPcrs.push(byteIndex * 8 + bit);
            }
        }
        if (!selectedPcrs.length)
            throw new TypeError('TPM quote selected no PCRs');
    ...
    if (cursor.offset !== data.length)
        throw new TypeError('TPMS_ATTEST has trailing bytes');
    return {
        magic,
        type,
        qualifiedSigner,
...
function quoteFailure(code, reason) {
    return { valid: false, code, reason };
}
function strictBase64url(value, name) {
    if (typeof value !== 'string' || !/^[A-Za-z0-9_-]+$/.test(value))
        throw new TypeError(`${name} is not base64url`);
    const decoded = Buffer.from(value, 'base64url');
    if (!decoded.length || decoded.toString('base64url') !== value)
        throw new TypeError(`${name} is not canonical base64url`);
    return decoded;
}
function expectedPcrKeys(parsed) {
    return parsed.pcrSelections.flatMap((selection) => selection.selectedPcrs.map((pcr) => `${selection.algorithm}:${pcr}`));
}
function reconstructPcrDigest(parsed, pcrValues, hashAlgorithm) {
...
function validateEventLog(eventLog, values, requireEventLog) {
    if (eventLog === null)
        return !requireEventLog;
    ...
    for (const item of eventLog) {
        if (!item || typeof item !== 'object' || Array.isArray(item))
            return false;
        const entry = item;
        if (JSON.stringify(Object.keys(entry).sort()) !== JSON.stringify(['bank', 'digest', 'pcr']))
            return false;
        if (typeof entry.bank !== 'string' || !Number.isInteger(entry.pcr) || typeof entry.digest !== 'string')
            return false;
        ...
        if (!bank || !/^[0-9a-f]+$/.test(entry.digest) || entry.digest.length !== bank.size * 2)
            return false;
...
export function tpm2KeyIdFromPublicKey(publicKeyPem) {
    const publicKey = createPublicKey(publicKeyPem);
    const der = publicKey.export({ format: 'der', type: 'spki' });
    return `tpm2:${createHash('sha256').update(der).digest('hex').slice(0, 32)}`;
}
export function verifyTpmQuote(evidence, signatureValue, options) {
    try {
        const fields = [
        ...
        if (!parsed.extraData.equals(nonce))
            return quoteFailure('TPM_NONCE_MISMATCH', 'quote nonce does not match challenge');
        if (!parsed.safe)
            return quoteFailure('TPM_QUOTE_MALFORMED', 'TPM clock was not safe when quoted');
        ...
        const reconstructed = reconstructPcrDigest(parsed, evidence.pcrValues, evidence.hashAlgorithm);
        ...
        if (publicKey.asymmetricKeyType !== 'rsa')
            return quoteFailure('TPM_AK_MISMATCH', 'registered AK is not RSA');
        const verifier = createVerify('sha256');
        verifier.update(quote);
        verifier.end();
        const signatureValid = verifier.verify({ key: publicKey, padding: constants.RSA_PKCS1_PADDING }, signature);
        if (!signatureValid)
            return quoteFailure('TPM_QUOTE_SIGNATURE', 'TPM quote signature is invalid');
