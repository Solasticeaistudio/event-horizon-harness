...
The 2026-07-24 reference run used `swtpm` 0.7.3 and `tpm2-tools` 5.6 and passed all 17 Executor Attestation tests with no skip. The persistent-handle workflow fixed by that run follows the upstream `tpm2_createak`/`tpm2_evictcontrol` pattern.
