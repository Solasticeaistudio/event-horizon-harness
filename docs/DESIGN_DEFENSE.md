...
`CapabilityBroker` signs canonical `CapabilityClaims` with an Ed25519 private key. In the process-separated harness the key exists in the signer service, outside the sacrificial executor; the executor is configured with only the corresponding public key and derived key ID. Development runs generate ephemeral keys. There is no production HSM or managed key service in this artifact.
...
The recorder appends fixed-size canonical events with global sequence, per-source sequence, previous hash, and event hash. It rescans before append and on recovery, detects modification, partial-record truncation, reordering, gaps, and linkage failure, and returns Ed25519-signed receipts. The containment certificate binds the verified chain tip and count plus attestation, capability, policy, image, teardown, egress, and assertion domains, then signs the canonical certificate with a separate Ed25519 key. Verification can pin the expected public key and key ID. Removal of a complete trailing suffix after recorder state loss requires an externally retained receipt or certificate checkpoint to detect; the local file alone cannot prove that its last valid event was globally final.

In the demo this is logical separation, not physical independence or one-way storage. A compromised host kernel, recorder process, recorder key, or certificate key remains capable of suppressing or fabricating evidence. Separately administered append-only storage and protected signing are future work.
