...
The default bridge uses `SqliteNoncePersistence`. Its conditional update is atomic across cooperating verifier processes that share one namespace and SQLite database on a local filesystem, and consumed state survives verifier restarts. Database unavailability, corruption, or an unsupported schema version fails closed. `NoncePersistence` and `Verifier.verify()` are awaitable so `RemoteNoncePersistence` can perform the same transition through the authenticated replay protocol without blocking network I/O. Remote response, epoch, checkpoint, or transport failure returns verifier-unavailable and cannot authorize the proof. The shipped remote service is a single-writer reference, not a multi-host consensus claim; see [Durable replay state](REPLAY_STATE.md) and [Authenticated remote replay protocol](REMOTE_REPLAY_PROTOCOL.md).
