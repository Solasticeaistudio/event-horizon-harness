...
RUN_DIR="$(mktemp -d "${TMPDIR:-/tmp}/hardproof-swtpm.XXXXXX")"
SERVER_PORT="${HARDPROOF_SWTPM_PORT:-2321}"
...
cleanup() {
  ...
  case "$RUN_DIR" in
    "${TMPDIR:-/tmp}"/hardproof-swtpm.*) rm -rf -- "$RUN_DIR" ;;
...
HARDPROOF_TPM_TCTI="$TCTI" \
HARDPROOF_TPM_WORKDIR="$RUN_DIR/ak" \
