import { decodeBundle, type Server, type VerificationSuccess } from '@event-horizon/attestation-sdk';
...
export function attestationMiddleware(options: { server: Server; nonceFrom?: (request: RequestLike) => string | undefined }) {
  return async (request: RequestLike, response: ResponseLike, next: NextLike): Promise<unknown> => {
    ...
    if (!encoded || !nonce) return response.status(401).json({ error: 'attestation_bundle_and_nonce_required' });
    try {
