...
test('simulator proof verifies', async () => {
  ...
  const result = verifier.verify(proof, { nonce, context });
...
test('nonce mismatch fails', async () => {
  ...
  const { nonce, context } = issueChallenge(verifier, deviceId);
  const proof = await prover.prove({ nonce });
  const result = verifier.verify(proof, { nonce: 'different-nonce-value-0000', context });
  assert.equal(result.valid, false);
...
test('proof replay fails', async () => {
  ...
  const { nonce, context } = issueChallenge(verifier, deviceId);
  const proof = await prover.prove({ nonce });
  assert.equal(verifier.verify(proof, { nonce, context }).valid, true);
  const replay = verifier.verify(proof, { nonce, context });
