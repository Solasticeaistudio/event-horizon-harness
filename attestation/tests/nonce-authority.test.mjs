...
function fixture(options = {}) {
  ...
  return {
    ...
    async issue(overrides = {}) {
      const issuedContext = { ...context, ...overrides };
      return {
        context: issuedContext,
        nonce: await verifier.nonceAuthority.issue(issuedContext),
      };
    },
  };
}
...
test('unknown nonce fails even when it is signed by the enrolled simulator', async () => {
  ...
  assert.equal(
    failureCode(await value.verifier.verify(proof, { nonce, context: value.context })),
    'NONCE_UNKNOWN',
  );
});
...
test('expired nonce transitions to expired and fails closed', async () => {
  ...
  const { nonce, context } = await value.issue();
  const proof = await value.prover.prove({ nonce });
  value.advance(1_000);
  assert.equal(failureCode(await value.verifier.verify(proof, { nonce, context })), 'NONCE_EXPIRED');
  assert.equal((await value.verifier.nonceAuthority.inspect(nonce))?.state, 'expired');
});
...
test('a consumed nonce cannot validate a second signed bundle', async () => {
  ...
  assert.equal((await value.verifier.verify(first, { nonce, context })).valid, true);
  assert.equal(failureCode(await value.verifier.verify(second, { nonce, context })), 'NONCE_REPLAY');
});
...
test('nonce issued for one executor cannot cross executor context', async () => {
  ...
  assert.equal(
    failureCode(await value.verifier.verify(proof, { nonce, context: wrongContext })),
    'NONCE_CONTEXT_MISMATCH',
  );
  assert.equal((await value.verifier.nonceAuthority.inspect(nonce))?.state, 'issued');
});
...
test('malformed nonce cannot be registered or accepted', async () => {
  ...
  assert.equal(
    failureCode(await value.verifier.verify(proof, { nonce: malformed, context: value.context })),
    'MALFORMED_NONCE',
  );
});
...
test('parallel verification attempts consume one nonce at most once', async () => {
  ...
  const results = await Promise.all(
    proofs.map(async (proof) => await value.verifier.verify(proof, { nonce, context })),
  );
  ...
  assert.equal((await value.verifier.nonceAuthority.inspect(nonce))?.state, 'consumed');
});
...
test('correct context permits exactly one successful verification', async () => {
  ...
  const { nonce, context } = await value.issue();
  const proof = await value.prover.prove({ nonce });
  const result = await value.verifier.verify(proof, { nonce, context });
